/*  ----------------------------------------------------------------<Prolog>-
    Name:       formio.c
    Title:      Form interface for C programs
    Package:    Studio

    Written:    1997/01/25  iMatix <tools@imatix.com>
    Revised:    2000/02/18

    Synopsis:   Provides functions to manage Studio forms.

    This software is copyright (c) 1996-2000 iMatix.  All rights reserved.
...
static char
    *error_prefix = "<TABLE><TR><TD BGCOLOR=\"#FF0000\">&gt;",
    *error_suffix = "</TABLE>";
...
static void  put_image       (VDESCR *stream, byte *block);
...
#define SELECT_name(b)     ((char *) ((b) + 7))
#define SELECT_value(b)    (SELECT_name  (b) + strlen (SELECT_name  (b)) + 1)
#define SELECT_nosel(b)    (SELECT_value (b) + strlen (SELECT_value (b)) + 1)
#define SELECT_first(b)    (SELECT_nosel (b) + strlen (SELECT_nosel (b)) + 1)

#define RADIO_attr(b)      ((b) [3])
#define RADIO_times(b)     ((b) [4])
#define RADIO_detail(b)    ((b) [5])
#define RADIO_column(b)    ((b) [6])
#define RADIO_name(b)      ((char *) ((b) + 7))
#define RADIO_value(b)     (RADIO_name  (b) + strlen (RADIO_name  (b)) + 1)
#define RADIO_nosel(b)     (RADIO_value (b) + strlen (RADIO_value (b)) + 1)

#define RLABEL_block(b)    ((b) [3] << 8) + ((b) [4])
#define RLABEL_index(b)    ((b) [5])
#define RLABEL_value(b)    ((char *) ((b) + 6))
...
/*  ---------------------------------------------------------------------[<]-
    ...
    To use live links, you must have set the form symbol table fields 'main'
    ...
    while (block_nbr < form-> defn-> block_count)
      {
        ...
        if (block_type == BLOCK_IF)     /*  Then we handle control blocks    */
          {
            if (empty_value (field_value (IF_field (block_ptr), 0)))
                skip_count = IF_scope (block_ptr);
          }
        else
        if (block_type == BLOCK_UNLESS)
          {
            if (!empty_value (field_value (UNLESS_field (block_ptr), 0)))
    ...
    if (tags-> input && within_input (field_nbr, index))
      {
        ...
        if (tags-> error)
            put_str (stream, error_prefix);

        put_fmt (stream,
            "<INPUT TYPE=%s NAME=\"%s\" SIZE=%d MAXLENGTH=%d VALUE=\"",
            tags-> input,
 ...
 *  button, and given an encoded name that indicates the original field and
    ...
    if (tags-> input && within_input (field_nbr, index))
      {
        ...
        put_fmt (stream,
            ...
            field_value (field_nbr, index) [0] == '1'? "CHECKED ": "",
            field_name (block_ptr, index));
      ...
      {
        value = field_value (field_nbr, index) [0] == '1'?
    ...
    if (tags-> input && within_input (field_nbr, index))
      {
        ...
        put_fmt (stream, "<SELECT NAME=\"%s\" SIZE=%d>\n",
                 field_name (block_ptr, index),
...
put_radio (VDESCR *stream, byte *block_ptr, int field_nbr, int index)
{
    ...
    byte
        *data_block;                    /*  Data block for radio field       */
    char
        *value;
    byte
        attr;                           /*  Field attribute                  */
    ...
    data_block  = cur_form-> defn-> blocks + RLABEL_block (block_ptr);
    ...
    if (tags-> input && within_input (field_nbr, index))
      {
        /*  Break if radio should be shown in column                         */
        if (RLABEL_index (block_ptr) > 1 && RADIO_column (data_block))
            put_str (stream, "<BR>");

        if (tags-> error)
            put_str (stream, error_prefix);

        put_fmt (stream, "<INPUT TYPE=RADIO VALUE=\"%d\" NAME=\"%s\"%s>%s\n",
                 RLABEL_index (block_ptr),
                 field_name (data_block, index),
                 RLABEL_index (block_ptr) == radio_value? " CHECKED": "",
                 RLABEL_value (block_ptr));

        if (tags-> error)
            put_str (stream, error_suffix);
      }
    else
    if (RADIO_detail (data_block))
      {
        /*  Break if radio should be shown in column                         */
        if (RLABEL_index (block_ptr) > 1 && RADIO_column (data_block))
            put_str (stream, "<BR>");
        ...
        value = xstrcpy (NULL,
                         RLABEL_index (block_ptr) == radio_value?
                             " (*) ": " (&nbsp;) ",
                         RLABEL_value (block_ptr),
                         NULL);
        ...
        if (attr == FATTR_OPTION)
            ...
            put_fmt (stream, tags-> output, value);

        mem_free (value);
      }
    else
    if (radio_value == 0 && RLABEL_index (block_ptr) == 1)
      {
        if (attr == FATTR_OPTION)
            put_option (stream, RADIO_nosel (data_block), field_nbr, index);
        else
            put_fmt (stream, tags-> output, RADIO_nosel (data_block));
      }
    else
    if (RLABEL_index (block_ptr) == radio_value)
      {
        if (attr == FATTR_OPTION)
            put_option (stream, RLABEL_value (block_ptr), field_nbr, index);
        else
            put_fmt (stream, tags-> output, RLABEL_value (block_ptr));
      }
}
    ...
    if (*ACTION_attr (block_ptr, index) == FACTION_ENABLED)
      {
        if (ACTION_type (block_ptr) == FTYPE_BUTTON)
            put_fmt (stream, "<INPUT TYPE=SUBMIT VALUE=\"%s\" NAME=\"%s\">\n",
                     label, name);
        else
        if (ACTION_type (block_ptr) == FTYPE_PLAIN)
          {
            if (cur_form-> JavaScript)
                ...
                put_fmt (stream,
                         "<A HREF=\"javascript:subconf('~A%d.%d','%s')\">%s</A>\n",
                         event, index, confirm, label);
            else
                /*  Encode as ~Annn.yyy=1                                    */
                put_fmt (stream, "<A HREF=\"#(uri)&~A%d.%d=1\">%s</A>\n",
                         event, index, label);
          }
        else
            ...
            if (cur_form-> JavaScript)
              {
                ...
                if (*rollover)
                  {                     /*  We call images ixxxxx            */
                    put_fmt (stream, " onMouseOver=\"show('i%s','%s')\"",
                                       name, rollover); 
                    put_fmt (stream, " onMouseOut=\"show('i%s','%s')\"",
                                       name, label);
                  }
                put_fmt   (stream, ">");
                put_image (stream, block_ptr);
                put_fmt   (stream, "</A>");
                mem_free  (alt_text);
              }
            else
              {
                /*  Encode as an image <INPUT field (no ALT is possible)     */
                put_fmt (stream,
                         "<INPUT TYPE=IMAGE BORDER=0 SRC=\"%s\" NAME=\"%s\" ALIGN=MIDDLE",
                         label, name);
...
static void
put_image (VDESCR *stream, byte *block_ptr)
{
    char
        *alt_text;
    ...
    const char *query)
{
     ...
     *  When JavaScript is enabled we may get the click action in the field
     *  called jsaction.
     */
 ...
 *  reset_attr -- internal
 *
 *  Resets feld transient attributes:
...
form_get_click (SYMBOL *symbol, ...)
{
    ...
    if (symbol-> name [0] == '~')       /*  Click fields start with ~        */
      {
...
{
    static char
        formatted [20];                 /*  Up to 64-bit value, 19 digits    */

    ASSERT (index < BLOCK_times (FIELD_block (field)));
    if (field < 0 || field >= cur_form-> defn-> field_count)
        return (0);

    sprintf (formatted, "%ld", new_value);
    return (fxputn_text (field, index, formatted));
}
...
fxputn_double (int field, int index, double new_value)
{
    ...
    sprintf (formatted, format, new_value);
    return (fxputn_text (field, index, formatted));
}
...
list_key (int field, int index, int item, char what)
{
    ...
    sprintf (key, "%02x.%02x.%02x.%c",
                  (byte) field, (byte) index, (byte) item, what);
    return (key);
}
    ...
    switch (BLOCK_type (block_ptr))     /*  Now handle the block             */
      {
        ...
        case BLOCK_ACTION:
            printf ("action     ");
            printf ("x%d type=%d event=%d show=%d height=%d width=%d label=%s\n",
